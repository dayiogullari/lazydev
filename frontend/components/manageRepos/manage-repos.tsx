...
import { Loader2, ChevronLeft, ChevronRight, Search, SortAsc, SortDesc } from "lucide-react";
  ...
  useEffect(() => {
    async function fetchRepos() {
      if (session?.accessToken) {
        try {
          setLoading(true);
          const { repos: adminRepos, pagination: paginationInfo } = await fetchAdminRepos(
            session.accessToken,
            currentPage,
            100,
          );
      ...
      {allRepos.length > 0 && (
        <div className="relative border-b border-zinc-800/50">
          <div className="flex gap-8 pb-2">
            <button onClick={() => setActiveTab("manage-repos")} className="relative px-1 py-3">
              <span
            ...
            <button onClick={() => setActiveTab("contract-builder")} className="relative px-1 py-3">
              <span
                className={`text-xl font-semibold transition-colors ${
          ...
          <>
                ...
                <div className="relative inline-block">
                  ...
                  <div className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                    {sortOption === "newest" || sortOption === "name-desc" ? (
                      <SortDesc size={16} className="text-zinc-400" />
                    ) : (
                      <SortAsc size={16} className="text-zinc-400" />
                    )}
                  </div>
                </div>
              </div>
            ...
            {filteredRepos.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-zinc-400">No repositories match your search.</p>
              </div>
            ) : (
              <RepoList repos={filteredRepos} onRepoClick={handleRepoClick} />
            )}

            {(pagination.hasNextPage || pagination.hasPreviousPage) && !searchQuery && (
              <div className="flex justify-center items-center gap-4 mt-6">
                ...
                <span className="text-zinc-400">
                  ...
                  {pagination.totalCount > 0 &&
                    ` of ${Math.ceil(pagination.totalCount / pagination.perPage)}`}
                </span>
