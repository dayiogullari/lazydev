...
export const RepoDetails: React.FC<RepoDetailsProps> = ({
  ...
  const [oldCommitmentResult, setOldCommitmentResult] = useState<RepoConfig | undefined>(undefined);
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-[#0A0A0A] p-6 rounded-lg shadow-lg border border-zinc-800 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-slate-300 mb-6">
                <p className="mb-4">
                  There are differences between your current configuration and the previously
                  committed configuration. Using the committed configuration is required for
                  linking.
                </p>
                ...
                <div className="p-4 bg-amber-400/10 border border-amber-400/20 rounded-lg mb-2">
                  <p className="text-amber-300 text-sm">
                    ...
                    <strong>committed configuration</strong> (green) instead of your current
                    configuration. Your current configuration changes will not be applied.
                  </p>
                </div>
              </div>
                      ...
                      <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-green-400/10 border border-green-400/20">
                        ...
                        <span
                          className={`text-xs font-medium ${
                            isLinked || isAlreadyLinked ? "text-green-400" : "text-orange-400"
                          }`}
                        >
                          {isLinked ? "Linked" : isAlreadyLinked ? "Already Linked" : "Linking"}
                        </span>
                      </div>
                    )}
                  </div>
