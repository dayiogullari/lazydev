...
for (let i = 0; i < APPLICATION_IDS.length; i++) clients.push(new ReclaimClient(APPLICATION_IDS[i], APPLICATION_SECRETS[i]))
...
app.get('/index', (req, res) => { return res.status(200).json({ message: "im running" }) })
...
app.post('/proof-user', async (req, res) => {
  try {
    ...
    reclaimApplicationIndex = reclaimApplicationIndex == APPLICATION_IDS.length - 1 ? 0 : ++reclaimApplicationIndex;
...
app.post('/proof-pr', async (req, res) => {
  ...
  try {
    const proof = await client.zkFetch(prUrl, {
      method: 'GET',
    }, {
      responseMatches: [
        {
    ...
    if (error.message == "Provider returned error 403\"") {
      return res.status(403).json({ message: 'forbidden', error_description: "this user doesnt have access to the repo, they are not the owner" })
    }
    if (error.message == "Provider returned error 404\"") {
      return res.status(404).json({ message: 'not_found', error_description: "this repo doesnt exist" })
    }
    res.status(500).json({ message: "database_error", error: error })
  }
})
...
app.post('/proof-repo-owner', async (req, res) => {
  ...
  if (!repo || !repoOwner || !githubUsername || !accessToken) return res.status(400).json({ message: 'bad_request' })
  ...
  try {
    ...
    if (error.message == "Provider returned error 401\"") {
      return res.status(401).json({ message: 'not_authorized', error_description: "bad token credentials" })
    }
    if (error.message == "Provider returned error 403\"") {
      return res.status(403).json({ message: 'forbidden', error_description: "this user doesnt have access to the repo, they are not the owner" })
    }
